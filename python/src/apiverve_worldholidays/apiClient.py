...
class WorldholidaysAPIClientError(Exception):
...
class ValidationError(WorldholidaysAPIClientError):
...
class WorldholidaysAPIClient:
    ...
    VALIDATION_RULES = {"country": {"type": "string", "required": True, "minLength": 2, "maxLength": 2}, "year": {"type": "integer", "required": False, "min": 2000, "max": 2050, "default": 2026}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the WorldholidaysAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises WorldholidaysAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise WorldholidaysAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[WorldholidaysAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns WorldHolidaysData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises WorldholidaysAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise WorldholidaysAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise WorldholidaysAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise WorldholidaysAPIClientError(f"Request failed: {str(e)}")
